/**
 * LVOAuthWidget — mounts a sign-in / sign-up / 2FA form.
...
const LVOAuthWidget = (function () {
  ...
  function injectStyle(container) {
    ...
    style.textContent = `
      ...
      .lvo-btn{width:100%;background:transparent;border:.5px solid #7F77DD;color:#7F77DD;font-family:'Cinzel',serif;font-size:.46rem;letter-spacing:.3em;padding:.8rem;cursor:pointer;text-transform:uppercase;margin-top:.4rem;transition:background .25s,color .25s}
  ...
  function mount(container, { division = 'Alliance', onAuthenticated } = {}) {
    ...
    function fieldEl(name, label, type, extra = {}) {
      ...
      f.appendChild(h('input', { name, type, autocomplete: type === 'password' ? 'current-password' : 'off', ...extra }));
      return f;
    }
    ...
    function renderAuthForm() {
      ...
      if (mode === 'signup') {
        ...
        row.appendChild(fieldEl('firstName', 'First Name', 'text'));
        row.appendChild(fieldEl('lastName', 'Last Name', 'text'));
        form.appendChild(row);
        form.appendChild(fieldEl('username', 'Username', 'text'));
      }
